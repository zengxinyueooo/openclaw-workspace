...
### 派活流程
1. **spawn 前**：调 `agent-runs-cli/spawn.sh` 写一条 status=spawned 记录，拿到 `RUN_ID`
2. `sessions_spawn` 派活给对应 agent
3. agent 读自己的 AGENTS.md → 找到 skill → 执行
4. **收到子 agent 回报后**：调 `agent-runs-cli/done.sh $RUN_ID returned "摘要"` 更新记录
5. 转述给皮皮
...
### 委派蚂蚁执行养号
收到「执行养号任务」时（由 `ant-account-nurturing` cron 每天 14:00 北京时间触发）：
1. spawn 蚂蚁执行：
   ```
   sessions_spawn(agentId="mcn-ant", task="执行养号任务", mode="run", runTimeoutSeconds=1800)
   ```
2. `sessions_yield` 等待蚂蚁结果
3. 收到结果后汇总告知皮皮：养了几个账号、各账号执行情况
4. 如果蚂蚁返回无需养号，回复「当前无需执行养号任务」
...
### 🔴 铁律：所有文件写操作必须用 `/mnt/openclaw/.openclaw/`，永远不要写 `/root/.openclaw/`

- `/root/.openclaw/workspace/` 会被 OpenClaw 注入 system prompt，这只是"让我能读到"，**不代表应该往那里写**
- 每次写文件前强制检查：路径是否以 `/mnt/openclaw/.openclaw/` 开头？不是就改掉，不要执行
- 读取用哪个路径都行；**写入只有一个合法路径：`/mnt/openclaw/.openclaw/`**

| 路径 | 写权限 | 说明 |
|------|--------|------|
| `/mnt/openclaw/.openclaw/workspace/` | ✅ 可写 | 正确的持久化路径，文件属主 catclaw |
| `/root/.openclaw/workspace/` | ❌ 禁止写 | root 属主，任何写操作都是错的 |

**结论：workspace 下的一切写操作（AGENTS.md、SOUL.md、MEMORY.md、日记、skill、脚本、任何文件）只能用 `/mnt/openclaw/.openclaw/workspace/` 路径。**
