...
LOG_FILE="$WORKSPACE/shared/logs/cron/nail_$(date +%Y%m%d).log"
...
# 记录Cron触发
audit_log "CRON_TRIGGERED" "美甲采集Cron触发"
echo "[$(date)] 美甲采集任务开始" >> "$LOG_FILE"

# 执行采集
cd "$WORKSPACE/skills/xhs-nail-collector"
export PATH="/tmp:/Users/lipengyu/Library/Python/3.9/bin:$PATH"
./collect.sh >> "$LOG_FILE" 2>&1

# 记录完成
audit_log "CRON_COMPLETED" "美甲采集任务完成"
echo "[$(date)] 美甲采集任务完成" >> "$LOG_FILE"
